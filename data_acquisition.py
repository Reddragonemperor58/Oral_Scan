...
class SensorDataReader:
    ...
    def read_data(self, duration=5):
        ...
        data_list = []
        # ... (rest of read_data as from when it handled sensor_point_id) ...
            ...
            try:
                ...
                if line:
                    try:
                        ...
                        if len(parts) == 5: # timestamp, tooth_id, sensor_point_id, force, contact_time
    ...
    def simulate_data(self, duration=5, num_teeth=16, num_sensor_points_per_tooth=4):
        ...
        for t in timestamps:
            for tooth_id in range(1, num_teeth + 1):
                ...
                # Introduce more variability based on tooth region for interesting patterns
                if 4 <= tooth_id <= 6 or 11 <= tooth_id <= 13: # Premolars/Canines
                    base_force_on_tooth *= np.random.uniform(0.7, 1.3)
                elif tooth_id <= 3 or tooth_id >= 14: # Molars
                    base_force_on_tooth *= np.random.uniform(0.9, 1.5)
                
                for sensor_point_idx in range(1, num_sensor_points_per_tooth + 1):
                    # Simulate gradient: make force vary across sensor points
                    # Example: sensor points at corners vs center
                    variation_factor = 1.0
                    if num_sensor_points_per_tooth == 4: # For a 2x2 grid
                        if sensor_point_idx == 1: variation_factor = np.random.uniform(0.7, 1.1) # Top-left
                        elif sensor_point_idx == 2: variation_factor = np.random.uniform(0.9, 1.3) # Top-right
                        elif sensor_point_idx == 3: variation_factor = np.random.uniform(0.6, 1.0) # Bottom-left
                        elif sensor_point_idx == 4: variation_factor = np.random.uniform(0.8, 1.2) # Bottom-right
                    else: # Simpler variation for other counts
                    ...
                    data_list.append({
                        'timestamp': t, 'tooth_id': tooth_id,
                        'sensor_point_id': sensor_point_idx,
...
if __name__ == '__main__':
    ...
    data = reader.simulate_data(duration=0.2, num_teeth=2, num_sensor_points_per_tooth=4)
    print(data.head(10))
